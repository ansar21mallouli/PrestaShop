...
module.exports = {
  ...
  sortCountry: function (selector, sortBy) {
    ...
    scenario('Check the sort of countries by "' + sortBy.toUpperCase() + '"', client => {
      test('should click on "Sort by ASC" icon', () => {
        ...
        return promise
          .then(() => client.waitForExistAndClick(Location.Country.sort_icon.replace('%B', sortBy).replace('%W', 2)));
      });
      test('should check that countries are well sorted by ASC', () => {
        for (let j = 0; j < parseInt(tab['number_countries']); j++) {
      ...
      test('should check that countries are well sorted by DESC', () => {
        for (let j = 0; j < parseInt(tab['number_countries']); j++) {
          promise = client.getTableField(selector, j, true);
        }
        return promise
