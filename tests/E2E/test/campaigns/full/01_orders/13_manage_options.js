...
scenario('Manage options', () => {
  ...
  scenario('Test Case 1: Check invoice generation when disabling the invoice option', client => {
    ...
    scenario('Disable invoice and check the invoice', client => {
      ...
      test('should click on "NO" option for "Enable invoices"', () => client.scrollWaitForExistAndClick(Invoices.disable_enable_invoice.replace('%STATUS', 0)));
      test('should close the symfony toolbar if exists', () => {
        return promise
          .then(() => client.isVisible(AddProductPage.symfony_toolbar))
          .then(() => {
      ...
      test('should click on "Save" button', () => client.waitForExistAndClick(Invoices.save_button, 2000));
      test('should check the success message', () => client.checkTextValue(Invoices.success_msg, 'Update successful'));
      test('should go to "Orders > Orders" page', () => client.goToSubtabMenuPage(Menu.Sell.Orders.orders_menu, Menu.Sell.Orders.orders_submenu));
      ...
      test('should click on "Orders" page', () => client.waitForExistAndClick(Menu.Sell.Orders.orders_submenu));
  ...
  scenario('Test Case 2: Check invoice generation when enabling the invoice option', client => {
    ...
    scenario('Enable invoice and check the PDF document', client => {
      test('should login successfully in the Back Office', () => client.linkAccess(URL + '/admin-dev'));
      test('should go to "Order - Invoice" page', () => client.goToSubtabMenuPage(Menu.Sell.Orders.orders_menu, Menu.Sell.Orders.invoices_submenu));
      test('should click on "YES" option for "Enable invoices"', () => client.scrollWaitForExistAndClick(Invoices.disable_enable_invoice.replace('%STATUS', 1)));
      test('should close the symfony toolbar if exists', () => {
        return promise
          .then(() => client.isVisible(AddProductPage.symfony_toolbar))
          .then(() => {
  ...
  scenario('Test Case 3: Check invoice data and order data when enabling the invoice option', client => {
  ...
  scenario('Test Case 4: Check downloaded invoice', client => {
    ...
    test('should click on "Save" button', () => client.waitForExistAndClick(Invoices.save_button));
    test('should check the success message', () => client.checkTextValue(Invoices.success_msg, 'Update successful'));
    /** Here we can not check if image is displayed
     * in pdf file by automated tests
     * */
  }, 'order');
  scenario('Test Case 5: Check "Invoice prefix" option', client => {
    test('should go to "Invoice" page', () => client.waitForExistAndClick(Menu.Sell.Orders.invoices_submenu));
  ...
  scenario('Test Case 6: Check "Add current year to invoice number" option', client => {
    test('should go to "Invoice" page', () => client.waitForExistAndClick(Menu.Sell.Orders.invoices_submenu));
  ...
  scenario('Test Case 7: Check invoice number option', () => {
  ...
  scenario('Test Case 8: Check invoice "footer text" and "legal free text" option', client => {
    test('should go to "Order - Invoice" page', () => client.goToSubtabMenuPage(Menu.Sell.Orders.orders_menu, Menu.Sell.Orders.invoices_submenu));
  ...
  scenario('Test Case 9: Check "Reset sequential invoice number at the beginning of the year" option', () => {
    scenario('Change the invoice number option and create order from the front office', client => {
      test('should go to "Invoice" page', () => client.goToSubtabMenuPage(Menu.Sell.Orders.orders_menu, Menu.Sell.Orders.invoices_submenu));
      test('should click on "Yes" option for "Reset sequential invoice number at the beginning of the year"', () => client.scrollWaitForExistAndClick(Invoices.reset_sequential_invoice_label.replace('%STATUS', 1)));
      test('should click on "Save" button', () => client.waitForExistAndClick(Invoices.save_button));
      test('should check the success message', () => client.checkTextValue(Invoices.success_msg, 'Update successful'));
      test('should login successfully in the Front Office', () => client.linkAccess(URL));
    }, 'order');
    ...
    scenario('Check the invoice number', client => {
      ...
      test('should set the "Machine time" date', () => {
        return promise
          .then(() => client.setMachineDate('02 JAN 2008 16:37:00'))
          .then(() => client.refresh());
      });
      test('should go to "Orders > Orders" page', () => client.goToSubtabMenuPage(Menu.Sell.Orders.orders_menu, Menu.Sell.Orders.orders_submenu));
      test('should go the created order', () => client.waitForExistAndClick(OrderPage.order_view_button.replace('%ORDERNumber', 1)));
      test('should change the order state to "Payment accepted"', () => client.changeOrderState(OrderPage, 'Payment accepted'));
      test('should click on "DOCUMENTS" tab', () => client.waitForExistAndClick(OrderPage.document_submenu));
      test('should verify the invoice number is "#PREFIX00001"', () => client.checkTextValue(OrderPage.download_invoice_button, '00001', 'contain'));
      test('should get the invoice name', () => client.getDocumentName(OrderPage.download_invoice_button));
      test('should download the invoice', async () => {
        await client.waitForExistAndClick(OrderPage.download_invoice_button);
        await client.pause(5000);
      });
      test('should check that the invoice file name contain "00001"', async () => client.checkDocument(global.downloadsFolderPath, global.invoiceFileName.replace('/', '-'), '00001'));
      test('should set the "Machine time" date', () => client.setMachineDate(dateFormat(dateNow, 'dd mmm yyyy HH:MM:ss')));
      /*test('should check that the invoice file name contain "00001"', async () => {
        return promise
          .then(() => client.checkDocument(global.downloadsFolderPath, global.invoiceFileName.replace('/', '-'), '00001'))
          .then(() => client.setMachineDate(dateFormat(dateNow, 'dd mmm yyyy HH:MM:ss')));
      });*/
    }, 'order');
  }, 'order');
  scenario('Test Case 10: Use the disk as cache for PDF invoices', client => {
    test('should login successfully in the Back Office', () => client.signInBO(AccessPageBO));
    test('should go to "Order - Invoice" page', () => client.goToSubtabMenuPage(Menu.Sell.Orders.orders_menu, Menu.Sell.Orders.invoices_submenu));
    test('should choice "yes" for "Use the disk as cache for PDF invoices" option', () => client.scrollWaitForExistAndClick(Invoices.cache_pdf_option));
    test('should close the symfony toolbar if exists', () => {
      return promise
        .then(() => client.isVisible(AddProductPage.symfony_toolbar))
        .then(() => {
    ...
    scenario('Change default invoice options', client => {
      test('should go to "Order - Invoice" page', () => client.goToSubtabMenuPage(Menu.Sell.Orders.orders_menu, Menu.Sell.Orders.invoices_submenu));
      test('should set "#OUT" in the prefix input', () => client.waitAndSetValue(Invoices.invoice_prefix_input, "#IN"));
      test('should click on "No" option for "Enable product image "', () => client.waitForExistAndClick(Invoices.disable_enable_image.replace('%STATUS', 0)));
      test('should click on "Yes" option for "Add current year to invoice number"', () => client.waitForExistAndClick(Invoices.disable_enable_current_number.replace('%STATUS', 0)));
      test('should click on "Yes" option for "Reset sequential invoice number at the beginning of the year"', () => client.scrollWaitForExistAndClick(Invoices.reset_sequential_invoice_label.replace('%STATUS', 0)));
      test('should set the "Footer text " input', () => client.waitAndSetValue(Invoices.footer_text_input, ''));
      test('should set the "legal free text" input', () => client.waitAndSetValue(Invoices.legal_free_text_input, ''));
      test('should click on "Save" button', () => client.waitForExistAndClick(Invoices.save_button));
      test('should check the success message', () => client.checkTextValue(Invoices.success_msg, 'Update successful'));
    }, 'order');
  }, 'order');
